...
export class RequestTypeComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.staticService.getProductMaster().subscribe(res => {
            ...
            if (this.productList) {
                if (otherSubCategoryData.length > 0) {
                    this.otherCategoryOldValue = otherSubCategoryData[0]?.subCategories[0]?.name;
                    this.otherCategoryId = otherSubCategoryData[0]?.subCategories[0]?.id;
                    if (this.productList.filter(item => item == this.otherCategoryId).length > 0) {
                        this.clientForm.addControl(
                            'otherProduct',
                            new FormControl('', [Validators.required, Validators.minLength(3)])
                        );
                        this.clientForm.patchValue({
                            otherProduct: otherSubCategoryData[0]?.subCategories[0]?.name
                        });
                        this.otherStatusSetSelected = true;
                    }
                }
    ...
    checkOtherValue() {
        this.staticService.getProductMaster().subscribe(res => {
            if (res.result[0]?.subCategories[0]?.name.length > 0) {
                let otherMasterValueInit = res.result[0]?.subCategories[0]?.name;
                this.clientForm.value.otherProduct = otherMasterValueInit;
            } else {
                return;
            }
        });
